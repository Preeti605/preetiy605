...
class Order:
    def __init__(self, OrderID, orderType, price, volume):
...
A STL multimap has been used for storing the orders sorted by their prices. Since multimap implemented using red black tree, the insert has runtime complexity of O(logn)
Time Complexity of Delete Order message
An unordered map is used to store order id to an iterator into the orders map. Since map is implemented using hash function, the lookup has runtime of O(1) and same is time complexity of erasing too i.e O(1). Thus the time complexity of Delete order is O(1).
